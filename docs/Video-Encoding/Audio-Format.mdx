...
- **The most compact surround format that supports Dolby Atmos**: **E-AC-3 JOC (Dolby Digital Plus with Atmos)** or **DTS:X** (also, object-based. Way less commmon) audio from streaming services like Netflix provide the best home theater multi-channel audio experience; while compressing to a typical percentage of ≈90% reduction in comparison to the lossless Atmos track.
...
### Audio Syntax with Av1an (Command-Line Encoding Front-End)
    Av1an uses FFmpeg to encode audio, but with the `-a` flag placed in front and quotations. In example:
    ```bash
