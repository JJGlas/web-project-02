...
Download: [MediaInfo](https://mediaarea.net/en/MediaInfo). Use it to check if your M2TS(`.m2ts`) or Matroska(`.mkv`) video file has any streams or metadata you want to add or exclude.
MediaInfo metadata example:

![MediaInfo](/img/MediaInfo.png)

### Create a Short Test Clip
Before you encode a full length video, test your encode commands. Create a 10 second test clip snippet with both low and high motion scenes, copied from the source. Videos can only be losslessly trimmed at selected keyframe points, not at each frame. There are two GUIs that make this quick and simple:
...
For including and excluding different audio tracks, subtitles, chapter files, setting defaults I like to use MKVToolNix GUI.
